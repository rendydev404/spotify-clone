...
import TrackCard from "@/components/TrackCard"; // Impor TrackCard dari file aslinya
import { TrackCardSkeleton } from "@/components/TrackCardSkeleton"; // Impor skeleton dari filenya sendiri
...
export default function HomePage() {
  ...
  const TracksGrid = ({ tracks, title }: { tracks: Track[]; title: string }) => (
    <div>
  ...
  return (
    <main className="bg-zinc-900 text-white min-h-screen p-4 md:p-8">
      ...
      {isLoading ? <SkeletonGrid /> : <TracksGrid tracks={discoverTracks} title="" />}
    </main>
  );
}